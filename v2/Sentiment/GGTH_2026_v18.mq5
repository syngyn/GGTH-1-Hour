...
input double InpSentimentMinConf      = 0.30;     // Minimum confidence to act (0=always, 1=never)
input int    InpSentimentMaxAgeSec    = 1800;     // Max age in seconds before sentiment ignored (1800=30min)
input double InpSentimentVetoBand     = 0.20;     // |score| must exceed this to veto opposite signal
...
void CGGTHExpert::DisplayInfo()
  {
   int fs  = InpFontSize;
   int lh  = fs + 7;                     // tight line height
   int pad = 8;                           // inner padding px
   int bw  = fs * 26;                    // box width scales with font size
   ...
   int bh = pad * 2 + content_rows * lh + 6;
   ...
   #define MKSEP(id, yy) \
     { if(ObjectFind(0,(id))<0) { \
       ...
       ObjectSetInteger(0,(id),OBJPROP_XDISTANCE, InpXOffset+1); \
       ObjectSetInteger(0,(id),OBJPROP_YDISTANCE, (yy)); \
       ObjectSetInteger(0,(id),OBJPROP_XSIZE, bw-2); \
       ObjectSetInteger(0,(id),OBJPROP_YSIZE, 1); \
       ObjectSetInteger(0,(id),OBJPROP_BGCOLOR, C'45,75,160'); \
       ObjectSetInteger(0,(id),OBJPROP_BORDER_TYPE, BORDER_FLAT); \
       ObjectSetInteger(0,(id),OBJPROP_COLOR, C'45,75,160'); }
...
//--- Background
   ...
   ObjectSetInteger(0, bg, OBJPROP_XDISTANCE, InpXOffset);
   ObjectSetInteger(0, bg, OBJPROP_YDISTANCE, InpYOffset);
   ObjectSetInteger(0, bg, OBJPROP_XSIZE,     fs * 26);
