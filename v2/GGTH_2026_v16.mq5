...
#property description "v1.11: TimeGMT() watchdog (was hardcoded +3hr offset), quoted-key JSON parse"
#property description "v1.12: Validate InpTradingTimeframe at Init (only H1/H4/D1 supported)"
#property description "v1.13: Configurable stale-prediction watchdog + per-trade journal CSV"
...
#property description "v1.16: Flat EA-signal preferred over nested JSON; adaptive confidence-floor gate"
#property strict
...
input int             InpStartHour                 = 8;           // Session Start Hour (UTC)
input int             InpEndHour                   = 17;          // Session End Hour (UTC)
...
input int             InpAveragingStep              = 150;         // Points between averaging positions
input int             InpMaxPositions              = 3;           // Max FIFO Stack depth
input double          InpLotMultiplier             = 1.0;         // Averaging Lot Multiplier
...
//    terms), and both gates must clear for a signal to survive.
   if(m_using_flat_signal && m_confidence_floor > 0.0)
     {
      double abs_change = MathAbs(selected_pred.change_pct);
   ...
   if(signal_buy)
     {
      ...
      string comment=StringFormat("GGTH v"+EA_VERSION_STR+" [%s] %s%.2f%% TP:%."+IntegerToString(_Digits)+"f",
                                  tf_name,
...
//--- Header
   ...
   CreateLabel("MLEA_Title",x_pos,y_pos,
               "║              GGTH ML PREDICTOR v"+EA_VERSION_STR+"                       ║",
...
//--- Watchdog status banner (v1.13). Shows three colours:
//      GREEN  — heartbeat received and within freshness threshold
//      RED    — heartbeat is stale; new entries blocked
//      ORANGE — no heartbeat yet (depends on InpFailClosedOnMissingHeartbeat)
//    The banner is the only chart-visible signal that the EA is currently
//    refusing to enter on stale data; without it, a silent watchdog block
//    looks identical to "no signal yet".
   if(InpShowWatchdogStatusOnChart && InpEnableStaleWatchdog && !InpStrategyTesterMode)
     {
      string wd_text;
      color  wd_color;
      if(m_watchdog_stale)
        {
         wd_text=StringFormat("║  [WATCHDOG] STALE  age=%dm  ENTRIES BLOCKED",
                              (m_watchdog_age_sec>=0 ? m_watchdog_age_sec/60 : -1));
         wd_color=clrRed;
        }
      else if(m_last_updated_utc<=0)
        {
         wd_text="║  [WATCHDOG] NO HEARTBEAT YET";
         wd_color=clrOrange;
        }
      else
        {
         wd_text=StringFormat("║  [WATCHDOG] OK  age=%dm  (limit %dm)",
                              (m_watchdog_age_sec>=0 ? m_watchdog_age_sec/60 : 0),
                              InpStalePredictionMaxMinutes);
         wd_color=clrLimeGreen;
        }
      while(StringLen(wd_text)<62) wd_text+=" ";
      wd_text+="║";
      CreateLabel("MLEA_Watchdog",x_pos,y_pos,wd_text,InpFontSize,wd_color);
      y_pos+=line_height+5;
     }
