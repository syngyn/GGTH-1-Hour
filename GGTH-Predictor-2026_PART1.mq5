...
#property description "ML-Based Trading EA with Multi-TF Prediction Pattern System"
#property description "Features: 7 Selectable Buy/Sell Patterns, Smart TP Selection"
#property description "Volatility-Based Risk Detection, Maximum Spacing Display"
...
//+------------------------------------------------------------------+
//| Enumeration for TP prediction preference                         |
//+------------------------------------------------------------------+
enum ENUM_TP_PREFERENCE
  {
   TP_PREF_1H=0,          // Prefer 1H Prediction
   TP_PREF_4H=1,          // Prefer 4H Prediction
   TP_PREF_1D=2,          // Prefer 1D Prediction
   TP_PREF_BEST=3         // Best Aligned (most conservative)
  };
...
input ENUM_TIMEFRAMES InpTradingTimeframe=PERIOD_H1;        // Chart Timeframe (for new bar / indicators)
input bool    InpEnableTrading=true;                        // Enable live trading
input int     InpMinPredictionPips=2;                       // Min prediction distance to confirm direction

//--- Prediction Pattern Selection (BUY patterns - SELL is opposite)
input group "=== Prediction Pattern Selection ==="
input bool    InpPattern1=true;                             // Pattern 1: 1H↑ 4H↑ 1D↑ (All UP)
input bool    InpPattern2=false;                            // Pattern 2: 1H↓ 4H↑ 1D↑
input bool    InpPattern3=false;                            // Pattern 3: 1H↑ 4H↓ 1D↑
input bool    InpPattern4=false;                            // Pattern 4: 1H↓ 4H↑ 1D↑
input bool    InpPattern5=false;                            // Pattern 5: 1H↑ 4H↓ 1D↑
input bool    InpPattern6=false;                            // Pattern 6: 1H↓ 4H↓ 1D↑
input bool    InpPattern7=false;                            // Pattern 7: 1H↑ 4H↓ 1D↓

//--- Take Profit Prediction Selection
input group "=== Take Profit & Stop Loss ==="
input bool    InpUsePredictedPrice=true;                    // Use predicted price as TP
input ENUM_TP_PREFERENCE InpTPPreference=TP_PREF_1D;       // TP Prediction Preference
...
private:
   ...
   //--- Prediction direction flags (updated each tick)
   bool              m_1H_up;
   bool              m_4H_up;
   bool              m_1D_up;

   //--- Matched pattern tracking for display
   int               m_matched_buy_pattern;
   int               m_matched_sell_pattern;
...
private:
   ...
   //--- Prediction direction helpers
   void              UpdatePredictionDirections();
   bool              CheckBuyPattern(int pattern_num);
   bool              CheckSellPattern(int pattern_num);
   double            GetAlignedTP(bool is_buy);
...
CGGTHExpert::CGGTHExpert() : m_symbol(InpSymbol),
                             ...
                             m_4H_up(false),
                             m_1D_up(false),
                             m_matched_buy_pattern(0),
                             m_matched_sell_pattern(0),
                             m_csv_1H_count(0),
                             m_csv_4H_count(0),
                             m_csv_1D_count(0),
                             m_last_bar_time(0),
...
//--- Validate at least one pattern is selected
   if(!InpPattern1 && !InpPattern2 && !InpPattern3 && !InpPattern4 &&
      !InpPattern5 && !InpPattern6 && !InpPattern7)
     {
      Print("ERROR: No prediction pattern selected! Enable at least one pattern.");
...
//--- Log active patterns
   ...
   if(InpPattern1) active_patterns+="1 ";
   if(InpPattern2) active_patterns+="2 ";
   if(InpPattern3) active_patterns+="3 ";
   if(InpPattern4) active_patterns+="4 ";
   if(InpPattern5) active_patterns+="5 ";
   if(InpPattern6) active_patterns+="6 ";
   if(InpPattern7) active_patterns+="7 ";

   Print("GGTH Predictor v2.00 initialized | Active Patterns: [",active_patterns,"]");
...
//--- Remove all chart objects
   ...
   Print("GGTH Predictor v2.00 deinitialized");
  }
...
//--- Update prediction directions
   if(predictions_loaded)
     {
      UpdatePredictionDirections();
      DisplayInfo();
     }
   else
      DisplayError();
...
//--- Use original position's TP for averaging orders
   ...
   string comment=StringFormat("ML EA v2.00 [AVG L%d] → TP:%.5f",level,tp);
...
//+------------------------------------------------------------------+
//| Load CSV lookup file for specific timeframe                       |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Update prediction directions based on current price               |
//+------------------------------------------------------------------+
void CGGTHExpert::UpdatePredictionDirections()
  {
   double point=SymbolInfoDouble(m_symbol,SYMBOL_POINT);
   double pip=point;
   if(_Digits==3 || _Digits==5)
   ...
   double min_dist=InpMinPredictionPips*pip;

//--- Determine direction for each prediction
//--- UP means prediction > current_price + min distance
//--- DOWN means prediction < current_price - min distance
   m_1H_up=(m_pred_1H.prediction>(m_current_price+min_dist));
   m_4H_up=(m_pred_4H.prediction>(m_current_price+min_dist));
   m_1D_up=(m_pred_1D.prediction>(m_current_price+min_dist));

//--- For DOWN: prediction must be meaningfully below price
   bool h1_down=(m_pred_1H.prediction<(m_current_price-min_dist));
   bool h4_down=(m_pred_4H.prediction<(m_current_price-min_dist));
   bool d1_down=(m_pred_1D.prediction<(m_current_price-min_dist));

//--- If not clearly UP and not clearly DOWN, treat as neutral (neither)
//--- m_1H_up etc already set above; we only need DOWN for sell patterns
//--- The CheckBuyPattern/CheckSellPattern functions use m_1H_up directly
//--- For "down" checks, we invert: !m_1H_up checks if NOT up
//--- But we need actual DOWN (below price), not just "not up"
//--- So we store down flags too - but simplify: if min pips is 0,
//--- then up/down is simply above/below price

//--- Recalculate with proper logic:
//--- If InpMinPredictionPips > 0, we need clear directional moves
//--- A prediction in the neutral zone (within min pips) should NOT trigger
   if(InpMinPredictionPips>0)
     ...
     {
      m_1H_up=(m_pred_1H.prediction>m_current_price);
      m_4H_up=(m_pred_4H.prediction>m_current_price);
      m_1D_up=(m_pred_1D.prediction>m_current_price);
     }
  }

//+------------------------------------------------------------------+
//| Check if a specific BUY pattern matches current predictions       |
//| Pattern definitions for BUY:                                      |
//|   1: 1H↑  4H↑  1D↑   (all up)                                   |
//|   2: 1H↓  4H↑  1D↑                                              |
//|   3: 1H↑  4H↓  1D↑                                              |
//|   4: 1H↓  4H↑  1D↑   (same as 2)                                |
//|   5: 1H↑  4H↓  1D↑   (same as 3)                                |
//|   6: 1H↓  4H↓  1D↑                                              |
//|   7: 1H↑  4H↓  1D↓                                              |
//+------------------------------------------------------------------+
bool CGGTHExpert::CheckBuyPattern(int pattern_num)
  {
   bool h1_down=!m_1H_up;
   bool h4_down=!m_4H_up;
   bool d1_down=!m_1D_up;

   switch(pattern_num)
     {
      case 1: return(m_1H_up  && m_4H_up  && m_1D_up);    // All UP
      case 2: return(h1_down  && m_4H_up  && m_1D_up);    // 1H↓ 4H↑ 1D↑
      case 3: return(m_1H_up  && h4_down  && m_1D_up);    // 1H↑ 4H↓ 1D↑
      case 4: return(h1_down  && m_4H_up  && m_1D_up);    // 1H↓ 4H↑ 1D↑
      case 5: return(m_1H_up  && h4_down  && m_1D_up);    // 1H↑ 4H↓ 1D↑
      case 6: return(h1_down  && h4_down  && m_1D_up);    // 1H↓ 4H↓ 1D↑
      case 7: return(m_1H_up  && h4_down  && d1_down);    // 1H↑ 4H↓ 1D↓
      default: return(false);
     }
  }

//+------------------------------------------------------------------+
//| Check if a specific SELL pattern matches (opposite of BUY)        |
//| Pattern definitions for SELL:                                     |
//|   1: 1H↓  4H↓  1D↓   (all down)                                 |
//|   2: 1H↑  4H↓  1D↓                                              |
//|   3: 1H↓  4H↑  1D↓                                              |
//|   4: 1H↑  4H↓  1D↓   (same as 2)                                |
...
//|   7: 1H↓  4H↑  1D↑                                              |
//+------------------------------------------------------------------+
bool CGGTHExpert::CheckSellPattern(int pattern_num)
  {
   bool h1_down=!m_1H_up;
   bool h4_down=!m_4H_up;
   bool d1_down=!m_1D_up;
   ...
   switch(pattern_num)
     {
      case 1: return(h1_down  && h4_down  && d1_down);    // All DOWN
      case 2: return(m_1H_up  && h4_down  && d1_down);    // 1H↑ 4H↓ 1D↓
      case 3: return(h1_down  && m_4H_up  && d1_down);    // 1H↓ 4H↑ 1D↓
      case 4: return(m_1H_up  && h4_down  && d1_down);    // 1H↑ 4H↓ 1D↓
      case 5: return(h1_down  && m_4H_up  && d1_down);    // 1H↓ 4H↑ 1D↓
      case 6: return(m_1H_up  && m_4H_up  && d1_down);    // 1H↑ 4H↑ 1D↓
      case 7: return(h1_down  && m_4H_up  && m_1D_up);    // 1H↓ 4H↑ 1D↑
      default: return(false);
     }
  }

//+------------------------------------------------------------------+
//| Get aligned TP price - only use predictions that match direction  |
//| For BUY: only use predictions that are UP (above current price)   |
//| For SELL: only use predictions that are DOWN (below current price)|
//+------------------------------------------------------------------+
double CGGTHExpert::GetAlignedTP(bool is_buy)
  {
   double point=SymbolInfoDouble(m_symbol,SYMBOL_POINT);
   double pip=point;
   if(_Digits==3 || _Digits==5)
...
//--- Build arrays of aligned prediction prices and their preference order
   double aligned_prices[];
   int    aligned_pref[];
   ArrayResize(aligned_prices,0);
   ArrayResize(aligned_pref,0);

   if(is_buy)
     {
      //--- For BUY: only use UP predictions as TP
      if(m_1H_up && m_pred_1H.prediction>m_current_price)
        {
         int s=ArraySize(aligned_prices);
         ArrayResize(aligned_prices,s+1);
         ArrayResize(aligned_pref,s+1);
         aligned_prices[s]=m_pred_1H.prediction;
         aligned_pref[s]=0; // TP_PREF_1H
        }
      if(m_4H_up && m_pred_4H.prediction>m_current_price)
        {
         int s=ArraySize(aligned_prices);
         ArrayResize(aligned_prices,s+1);
         ArrayResize(aligned_pref,s+1);
         aligned_prices[s]=m_pred_4H.prediction;
         aligned_pref[s]=1; // TP_PREF_4H
        }
      if(m_1D_up && m_pred_1D.prediction>m_current_price)
        {
         int s=ArraySize(aligned_prices);
         ArrayResize(aligned_prices,s+1);
         ArrayResize(aligned_pref,s+1);
         aligned_prices[s]=m_pred_1D.prediction;
         aligned_pref[s]=2; // TP_PREF_1D
        }
     }
   else
     {
      //--- For SELL: only use DOWN predictions as TP
      if(!m_1H_up && m_pred_1H.prediction<m_current_price)
        {
         int s=ArraySize(aligned_prices);
         ArrayResize(aligned_prices,s+1);
...
//--- No aligned predictions available
   if(ArraySize(aligned_prices)==0)
      return(0);

//--- Select based on preference
   if(InpTPPreference!=TP_PREF_BEST)
     {
      //--- Try to use the preferred timeframe
      for(int i=0; i<ArraySize(aligned_pref); i++)
        {
         if(aligned_pref[i]==(int)InpTPPreference)
            return(aligned_prices[i]*InpTPMultiplier);
        }
      //--- Preferred not available, fall through to best aligned
     }

//--- TP_PREF_BEST: use most conservative (closest to current price)
   double best_price=aligned_prices[0];
   double best_dist=MathAbs(aligned_prices[0]-m_current_price);

   for(int i=1; i<ArraySize(aligned_prices); i++)
     {
      double dist=MathAbs(aligned_prices[i]-m_current_price);
      if(dist<best_dist)
        {
         best_dist=dist;
         best_price=aligned_prices[i];
        }
     }

   return(best_price*InpTPMultiplier);
  }

//+------------------------------------------------------------------+
//| Check for trade signal - PATTERN-BASED VERSION                   |
//+------------------------------------------------------------------+
...
//--- Need valid predictions for all three timeframes
   if(m_pred_1H.prediction<=0 || m_pred_4H.prediction<=0 || m_pred_1D.prediction<=0)
      return;
...
//--- Check all enabled patterns for BUY signal
   bool signal_buy=false;
   bool signal_sell=false;
   m_matched_buy_pattern=0;
   m_matched_sell_pattern=0;

   bool patterns_enabled[8];
   patterns_enabled[0]=false; // unused index 0
   patterns_enabled[1]=InpPattern1;
   patterns_enabled[2]=InpPattern2;
   patterns_enabled[3]=InpPattern3;
   patterns_enabled[4]=InpPattern4;
   patterns_enabled[5]=InpPattern5;
   patterns_enabled[6]=InpPattern6;
   patterns_enabled[7]=InpPattern7;

//--- Scan for BUY patterns first
   for(int p=1; p<=7; p++)
     {
      if(patterns_enabled[p] && CheckBuyPattern(p))
        {
         signal_buy=true;
         m_matched_buy_pattern=p;
         if(InpShowDebug)
            Print("✓ BUY Pattern ",p," matched | 1H:",
                  (m_1H_up?"UP":"DN")," 4H:",(m_4H_up?"UP":"DN"),
                  " 1D:",(m_1D_up?"UP":"DN"));
         break; // First matching pattern wins
        }
     }

//--- If no BUY, scan for SELL patterns
   if(!signal_buy)
     {
      for(int p=1; p<=7; p++)
        {
         if(patterns_enabled[p] && CheckSellPattern(p))
           {
            signal_sell=true;
            m_matched_sell_pattern=p;
            if(InpShowDebug)
               Print("✓ SELL Pattern ",p," matched | 1H:",
                     (m_1H_up?"UP":"DN")," 4H:",(m_4H_up?"UP":"DN"),
                     " 1D:",(m_1D_up?"UP":"DN"));
            break;
           }
...
//--- Use predicted price as TP (with alignment check)
   if(InpUsePredictedPrice)
     {
      tp_price=GetAlignedTP(signal_buy);

      //--- If no aligned TP available, fall back to fixed pips
      if(tp_price==0)
        {
         if(InpShowDebug)
            Print("No aligned prediction for TP, using fixed ",InpTakeProfitPips," pips");
...
//--- Build pattern info for comment
   string pattern_info="";
   if(signal_buy && m_matched_buy_pattern>0)
      pattern_info=StringFormat("P%d",m_matched_buy_pattern);
   else if(signal_sell && m_matched_sell_pattern>0)
      pattern_info=StringFormat("P%d",m_matched_sell_pattern);
   ...
   if(signal_buy)
     {
      ...
      double tp=(tp_price>0) ? tp_price : ask+tp_distance;

      if(tp<=ask)
         return;

      string comment=StringFormat("ML v2.00 [%s] 1H:%s 4H:%s 1D:%s → TP:%.5f",
                                  pattern_info,
                                  (m_1H_up?"↑":"↓"),
                                  (m_4H_up?"↑":"↓"),
                                  (m_1D_up?"↑":"↓"),
      ...
      if(m_trade.Buy(lot_size,m_symbol,ask,sl,tp,comment))
        {
         ...
         Print("✓ BUY [Pattern ",pattern_info,"] at ",ask,
               " | TP: ",tp," | SL: ",sl,
               " | 1H:",(m_1H_up?"UP":"DN"),
               " 4H:",(m_4H_up?"UP":"DN"),
               " 1D:",(m_1D_up?"UP":"DN"));
   ...
   else if(signal_sell)
     {
      ...
      double tp=(tp_price>0) ? tp_price : bid-tp_distance;
      ...
      if(m_trade.Sell(lot_size,m_symbol,bid,sl,tp,comment))
        {
         m_last_trade_time=TimeCurrent();
         Print("✓ SELL [Pattern ",pattern_info,"] at ",bid,
               " | TP: ",tp," | SL: ",sl,
               " | 1H:",(m_1H_up?"UP":"DN"),
               " 4H:",(m_4H_up?"UP":"DN"),
               " 1D:",(m_1D_up?"UP":"DN"));
...
//+------------------------------------------------------------------+
//| Display information - MAXIMUM SPACING VERSION with Pattern Info   |
//+------------------------------------------------------------------+
...
//--- Header
   ...
   CreateLabel("MLEA_Title",x_pos,y_pos,
               "║          GGTH ML PREDICTOR v2.00 (PATTERN)                 ║",
...
//--- Active patterns display
   ...
   if(InpPattern1) pat_text+="[1] ";
   if(InpPattern2) pat_text+="[2] ";
   if(InpPattern3) pat_text+="[3] ";
   if(InpPattern4) pat_text+="[4] ";
   if(InpPattern5) pat_text+="[5] ";
   if(InpPattern6) pat_text+="[6] ";
   if(InpPattern7) pat_text+="[7] ";
   while(StringLen(pat_text)<62) pat_text+=" ";
   pat_text+="║";
   CreateLabel("MLEA_Patterns",x_pos,y_pos,pat_text,InpFontSize,clrCyan);
   y_pos+=line_height+5;

//--- Current direction summary
   string dir_text=StringFormat("║     Directions:  1H:%s  4H:%s  1D:%s",
                                (m_1H_up?"UP  ":"DOWN"),
                                (m_4H_up?"UP  ":"DOWN"),
                                (m_1D_up?"UP  ":"DOWN"));
   while(StringLen(dir_text)<62) dir_text+=" ";
   dir_text+="║";

   color dir_color=clrYellow;
   if(m_1H_up && m_4H_up && m_1D_up) dir_color=clrLimeGreen;
   if(!m_1H_up && !m_4H_up && !m_1D_up) dir_color=clrRed;

   CreateLabel("MLEA_Directions",x_pos,y_pos,dir_text,InpFontSize+1,dir_color);
   y_pos+=line_height+10;
